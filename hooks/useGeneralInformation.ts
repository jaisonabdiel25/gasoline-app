...
export const useGeneralInformation = () => {
  ...
  const { totalPayments, totalAmountPayments, totalVehicles } = useMemo(() => {
    return {
      ...
      totalVehicles: generalInformation.length,
    };
  }, [generalInformation]);
  ...
  return { loading, totalPayments, totalAmountPayments, totalVehicles, totalAmountCurrentMonth };
};
